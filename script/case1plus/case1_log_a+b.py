...
# -------读取数据-------
# ---------------------
with open('./data_log/case1plus/infs_intra.log', 'r', encoding='utf-8') as f:
    content = f.readlines()
...
def clctPrfData (lst_index, colum_index):
    ...
    for i in lst_index:
        ...
        lst_pub_sub_info[msg_value-1, 0] = msg_value
        lst_pub_sub_info[msg_value-1, colum_index] = start_time
...
idx_get = [x for x in range(len(content)) if ('[spin]->[get_next_executable]') in content[x].lower()]
idx_exe = [x for x in range(len(content)) if ('[spin]->[execute_any_executable]') in content[x].lower()]

idx_pub = [x for x in range(len(content)) if ('pb1') in content[x].lower()]
idx_sub = [x for x in range(len(content)) if ('sb0') in content[x].lower()]

# ->Prepare: step2
# --> Data interprets: PROFILE |msg_value|pub_start|sub1_start|sub2_start|
lst_pub_sub_info= np.zeros([len(idx_pub), 2+1], dtype = int)


clctPrfData (idx_pub, 1)
clctPrfData (idx_sub, 2)

lst_pub_sub_info[329,:] = lst_pub_sub_info[328,:]
